...
const isOutsideClicked = (event) => {
    ...
    return !(sidebarEl.isSameNode(event.target) || sidebarEl.contains(event.target) || topbarEl.contains(event.target));
};
...
const searchMenu = (event) => {
    setTimeout(() => {
        ...
        for (let menu of allMenuList.value) {
            let filtered = FilterService.filter(menu.items, ['label'], query, FilterMatchMode.CONTAINS);
        ...
        if (inputValue != null && inputValue.length > 0) {
            ...
            filteredArray.forEach((item) => {
                ...
                if (menuName.toLowerCase().includes(inputValue.toLowerCase())) {
                    ...
                    item.innerHTML = menuName.replace(subs, `<span style=color:red; >` + subs + `</span>`);
                }
            });
        }
    }, 100);
...
<template>
    <div class="layout-topbar">
        <span class="p-link layout-menu-button layout-topbar-button" @click="onGoHome()"> CRM FrameWork </span>
        ...
        <div class="layout-topbar-menu" :class="topbarMenuClasses">
            ...
            <button v-tooltip.bottom="{ value: '마이페이지', showDelay: 200, hideDelay: 200 }" @click="onTopBarMenuButton()" class="p-link layout-topbar-button">
                <i class="pi pi-user"></i>
                <span>마이페이지</span>
            </button>
            <button v-tooltip.bottom="{ value: '로그아웃', showDelay: 200, hideDelay: 200 }" @click="goLogOut()" class="p-link layout-topbar-button">
